...
import { AutomationFormField } from './AutomationFormBuilder';

// ================== PRICING TYPES ==================

export interface AutomationPriceItem {
  id: string;
  label: string;
...
export interface AutomationPartialPayment {
  enabled: boolean;
...
export interface AutomationPricingConfig {
  enabled: boolean;
  basePrice: number;
  items: AutomationPriceItem[];
  partialPayment?: AutomationPartialPayment;
}

interface AutomationPricingConfigBuilderProps {
  pricingConfig: AutomationPricingConfig;
  onPricingConfigChange: (config: AutomationPricingConfig) => void;
...
export function migrateAutomationPricingConfig(config: any): AutomationPricingConfig {
  if (!config) {
    return { enabled: false, basePrice: 0, items: [] };
  }
  
  if (Array.isArray(config.items)) {
    return config as AutomationPricingConfig;
  }
...
export function AutomationPricingConfigBuilder({
  pricingConfig: rawConfig,
  onPricingConfigChange,
  formFields
}: AutomationPricingConfigBuilderProps) {
  const [expandedItems, setExpandedItems] = useState<Record<string, boolean>>({});
  
  const pricingConfig = migrateAutomationPricingConfig(rawConfig);

  const updateConfig = (updates: Partial<AutomationPricingConfig>) => {
    onPricingConfigChange({ ...pricingConfig, ...updates });
  };

  const addPriceItem = () => {
    const newItem: AutomationPriceItem = {
  ...
  const updatePriceItem = (itemId: string, updates: Partial<AutomationPriceItem>) => {
  ...
  const selectableFields = formFields.filter(f => 
    ['select', 'number', 'checkbox'].includes(f.type)
  );
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Configure dynamic pricing based on form field selections.
        </CardDescription>
      </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Default base fee applied to all form submissions
            </p>
          </div>
                        ...
                        <div className="p-3 pt-0 space-y-3 border-t">
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              placeholder="e.g., Premium Option, Add-on"
                              className="h-8"
                            />
                          </div>
                          ...
                          {/* Price Mapping for Select Fields */}
                          {item.type === 'field_price' && item.fieldId && 
                           formFields.find(f => f.id === item.fieldId)?.type === 'select' && (
